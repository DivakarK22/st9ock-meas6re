...
const { researchCompany } = require('./providers/news');
const { findInstrument } = require('./universe');
...
function createApp() {
  ...
  app.get('/api/analyze', async (req, res) => {
    try {
      ...
      const result = await analyzeInstrument(symbol, horizon, { withNews: true });
      res.json(result);
    } catch (error) {
  ...
  app.get('/api/research', async (req, res) => {
    try {
      ...
      const research = await researchCompany(instrument);
      res.json({ instrument, research });
    } catch (error) {
      res.status(502).json({ error: error.message });
    }
  });

  app.get('/api/daily-suggestion', async (req, res) => {
    try {
      const horizon = req.query.horizon === 'longterm' ? 'longterm' : 'intraday';
      const exchange = ['NSE', 'BSE', 'ALL'].includes(req.query.exchange)
        ? req.query.exchange
        : 'NSE';
      const limit = Math.min(80, Math.max(10, Number(req.query.limit) || 80));
      const count = Math.min(15, Math.max(1, Number(req.query.count) || 10));
      const result = await dailySuggestion({ horizon, exchange, limit, count });
      res.json(result);
    } catch (error) {
      res.status(502).json({ error: error.message });
    }
