...
const { fetchAnnualReportPack, annualReportBlurb } = require('./annualReports');
...
async function researchCompany(instrument) {
  ...
  if (mode === 'off') {
    return {
      ...
      profile: null,
      articles: [],
      annualReports: { reports: [], highlights: null },
      outlook: 'unknown',
      score: 0,
      themes: [],
  ...
  if (mode === 'demo') {
    ...
    const annualReports = await fetchAnnualReportPack(instrument);
    ...
    const packed = summarizeResearch({
      ...
      articles,
      score,
      annualBlurb: annualReportBlurb(annualReports),
    });
    return {
      source: 'demo',
      profile: { extract: packed.summary },
      articles,
      annualReports,
      ...packed,
    };
  ...
  const [articles, profile, annualReports] = await Promise.all([
    fetchGoogleNews(instrument),
    fetchWikipediaProfile(instrument),
    fetchAnnualReportPack(instrument),
  ]);
  ...
  return {
    source: articles.length || profile || annualReports.reports?.length
      ? 'google-news+wikipedia+nse-annual-report'
      : 'none',
    profile,
    articles,
    annualReports,
