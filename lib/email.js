...
function formatPickLine(row, index) {
  const n = index + 1;
  const outlook = row.research?.outlook ? ` · news ${row.research.outlook}` : '';
  return `${n}. ${row.exchange}:${row.symbol} — ${row.action} · buy ${rangeText(row.buyRange)} · window ${windowText(row.timing)} · sell by ${row.timing?.sellBy || '—'}${outlook}`;
}
...
function pickBlock(title, suggestion) {
  const session = suggestion.session
    ...
    : '';
  const rows = suggestion.picks?.length
    ? suggestion.picks
    : suggestion.pick
      ? [suggestion.pick]
      : [];
  if (!rows.length) {
    return [title, session, suggestion.headline || 'No names today', ''].join('\n');
  }
  return [
    title,
    session,
    suggestion.headline,
    rows.map((row, index) => formatPickLine(row, index)).join('\n'),
    '',
  ]
    .filter((line) => line !== '')
    .join('\n');
...
function composeDailyEmail({ intraday, longterm, to }) {
  const sessionDate = longterm?.session?.date || intraday?.session?.date || 'today';
  const count =
    longterm?.picks?.length ||
    (longterm?.pick ? 1 : 0) ||
    intraday?.picks?.length ||
    (intraday?.pick ? 1 : 0) ||
    0;
  const headline = count
    ? `${count} stocks to check and buy · ${sessionDate}`
    : `Daily suggestion ${sessionDate}`;
  const subject = `NSE daily suggestion · ${headline}`;
  const text = [
...
function composeDailyMarkdown({ intraday, longterm }) {
  const sessionDate = longterm?.session?.date || intraday?.session?.date || 'today';
  const rows = longterm?.picks?.length
    ? longterm.picks
    : longterm?.pick
      ? [longterm.pick]
      : intraday?.picks?.length
  ...
  const headline = rows.length
    ? `**${rows.length} stocks to check and buy** · ${sessionDate}`
    : `Daily suggestion ${sessionDate}`;
  const markdown = [
    `## 10 stocks to check and buy · ${sessionDate}`,
    '',
    headline,
    '',
    '_Check on Kite before you buy. Not a forecast of future returns._',
    '',
    '```',
    pickBlock('LONG TERM (primary buy list)', longterm || { picks: rows, session: null, headline: '' }),
    pickBlock('INTRADAY (same-session tape)', intraday || { picks: [], session: null, headline: '' }),
    '```',
    '',
    'Delivered in Cursor (no email).',
  ].join('\n');
  return { title: `10 stocks to check and buy · ${sessionDate}`, headline, markdown };
}
